...
class TestPageTitles:
    ...
    def test_clutches_title(self, client):
        ...
        assert "hatchery.onStatusTick" in html
        assert "library-status-cue-icon" in html
...
class TestLibraryConnectionsPane:
    ...
    def test_library_api_upsert_and_delete_connection(self, client, tmp_path, monkeypatch):
        ...
        assert len(library_registry_lib.list_connections()) == 1

        library_registry_lib.upsert_binding(
            {
                "id": "b1",
    ...
    def test_library_api_upsert_binding(self, client, tmp_path, monkeypatch):
        ...
        assert put.status_code == 200
        assert put.get_json()["ok"] is True
        assert library_registry_lib.list_bindings(domain="scripts")[0]["filter"] == "*.ps1"

        deleted = client.delete("/api/library/bindings/scripts/b1")
        assert deleted.status_code == 200
        assert library_registry_lib.list_bindings(domain="scripts") == []
    ...
    def test_library_cache_reattach_requires_binding(self, client, tmp_path, monkeypatch):
        """API rejects connection-only reattach and missing binding_id (#363)."""
        share = tmp_path / "share"
        share.mkdir()
        (share / "tool.sh").write_text("#!/bin/sh\n", encoding="utf-8")
        data = tmp_path / "data"
        ...
        assert no_binds.status_code == 400
        assert "Settings" in no_binds.get_json()["error"]
        ...
        assert "Select a Library binding" in missing.get_json()["error"]
        ...
        assert tested.get_json()["tested"] is True
